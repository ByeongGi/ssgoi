...
<Tabs items={[{ label: "React", value: "react" }, { label: "Svelte", value: "svelte" }, { label: "Vue", value: "vue" }, { label: "Angular", value: "angular" }]}>
  ...
  <TabPanel value="angular">
    ...
    import type { TransitionParams } from '@ssgoi/core';
    ...
    @Component({
      ...
      template: `
        <!-- Fade transition -->
        <div [ssgoiTransition]="fadeTransition">
          Fade effect
        </div>

        <!-- Scale transition -->
        <div [ssgoiTransition]="scaleTransition">
          Scale effect
        </div>

        <!-- Blur transition -->
        <div [ssgoiTransition]="blurTransition">
          Blur effect
        </div>

        <!-- Slide transition (with direction) -->
        <div [ssgoiTransition]="slideTransition">
          Slide effect
        </div>

        <!-- Fly transition (custom position) -->
        <div [ssgoiTransition]="flyTransition">
          Fly effect
        </div>

        <!-- Rotate transition -->
        <div [ssgoiTransition]="rotateTransition">
          Rotate effect
        </div>

        <!-- Bounce transition -->
        <div [ssgoiTransition]="bounceTransition">
          Bounce effect
        </div>
      `
    })
    export class ElementTransitionsComponent {
      fadeTransition: TransitionParams = { key: 'fade-element', ...fade({ spring: { stiffness: 300, damping: 30 } }) };
      scaleTransition: TransitionParams = { key: 'scale-element', ...scale({ spring: { stiffness: 300, damping: 30 } }) };
      blurTransition: TransitionParams = { key: 'blur-element', ...blur({ amount: 10, spring: { stiffness: 300, damping: 30 } }) };
      slideTransition: TransitionParams = { key: 'slide-element', ...slide({ direction: 'left', spring: { stiffness: 300, damping: 30 } }) };
      flyTransition: TransitionParams = { key: 'fly-element', ...fly({ x: 200, y: -50, spring: { stiffness: 300, damping: 30 } }) };
      rotateTransition: TransitionParams = { key: 'rotate-element', ...rotate({ spring: { stiffness: 300, damping: 30 } }) };
      bounceTransition: TransitionParams = { key: 'bounce-element', ...bounce({ spring: { stiffness: 300, damping: 30 } }) };
    }
    ```
  </TabPanel>
</Tabs>
...
<Tabs items={[{ label: "React", value: "react" }, { label: "Svelte", value: "svelte" }, { label: "Vue", value: "vue" }, { label: "Angular", value: "angular" }]}>
  ...
  <TabPanel value="angular">
    ...
    @Component({
      ...
      imports: [SsgoiTransitionDirective],
      template: `
        <div [ssgoiTransition]="transitionConfig">
          Content
        </div>
      `,
    })
